...
const fetchStations = async () => {
  ...
  try {
    ...
    toast.error('Failed to load stations from server.')
  } finally {
    loading.value = false
  }
}
...
const validateForm = () => {
  let isValid = true
  errors.value = { name: '', ip_address: '', location: '', drawer_safety_limit: '' }
  ...
  if (!form.value.name || form.value.name.trim() === '') {
    ...
    errors.value.name = 'Station name must be at least 3 characters.'
    isValid = false
  } else if (form.value.name.length > 255) {
    errors.value.name = 'Station name must not exceed 255 characters.'
    isValid = false
  }

  // IP Address Validation
  if (form.value.ip_address && form.value.ip_address.trim() !== '') {
    const trimmedIp = form.value.ip_address.trim()
    const ipRegex = /^(?:[0-9]{1,3}\.){3}[0-9]{1,3}$/
    if (!ipRegex.test(trimmedIp)) {
      errors.value.ip_address = 'Please enter a valid IPv4 address (e.g. 192.168.1.100).'
      isValid = false
    } else {
      const parts = trimmedIp.split('.')
      const invalidOctet = parts.some(part => {
        const num = parseInt(part, 10)
        return isNaN(num) || num < 0 || num > 255 || String(num) !== part
      })
      if (invalidOctet) {
        errors.value.ip_address = 'Each octet of the IP address must be between 0 and 255.'
        isValid = false
      }
    }
  }

  // Drawer Safety Limit Validation
  if (form.value.drawer_safety_limit !== '' && form.value.drawer_safety_limit !== null && form.value.drawer_safety_limit !== undefined) {
    const val = Number(form.value.drawer_safety_limit)
    if (isNaN(val) || val < 0) {
      errors.value.drawer_safety_limit = 'Drawer safety limit must be a positive number.'
      isValid = false
    }
  }

  // Location Validation
  if (form.value.location && form.value.location.length > 255) {
    errors.value.location = 'Location must not exceed 255 characters.'
    isValid = false
  }
...
const handleSubmit = async () => {
  ...
  const limitValue = form.value.drawer_safety_limit !== '' && form.value.drawer_safety_limit !== null
    ? Number(form.value.drawer_safety_limit)
    : null

  const payload = {
    name: form.value.name,
    ip_address: form.value.ip_address && form.value.ip_address.trim() !== '' ? form.value.ip_address.trim() : null,
    location: form.value.location && form.value.location.trim() !== '' ? form.value.location.trim() : null,
    drawer_safety_limit: limitValue,
    is_active: form.value.is_active ? 1 : 0
  }

  try {
    if (isEditMode.value) {
      await api.put(`/stations/${editId.value}`, payload)
...
const handleEdit = (station) => {
  ...
  form.value = {
    ...
    drawer_safety_limit: station.drawer_safety_limit !== null ? station.drawer_safety_limit : null,
    is_active: station.is_active === 1 || station.is_active === true
  }
}
...
// Filtered stations computed property
const filteredStations = computed(() => {
  if (!searchQuery.value) return stations.value
  const query = searchQuery.value.toLowerCase().trim()
  return stations.value.filter(station => 
    station.name.toLowerCase().includes(query) || 
    (station.ip_address && station.ip_address.toLowerCase().includes(query)) ||
...
// Paginated stations computed property
const paginatedStations = computed(() => {
  const startIndex = (currentPage.value - 1) * pageSize.value
  const endIndex = startIndex + pageSize.value
  return filteredStations.value.slice(startIndex, endIndex)
})

// Total pages computed property
const totalPages = computed(() => {
  return Math.ceil(filteredStations.value.length / pageSize.value) || 1
})
...
const startRange = computed(() => {
  if (filteredStations.value.length === 0) return 0
  return (currentPage.value - 1) * pageSize.value + 1
})

const endRange = computed(() => {
  const calculatedEnd = currentPage.value * pageSize.value
  return Math.min(calculatedEnd, filteredStations.value.length)
})

// Currency/Limit formatter (Indonesian Rupiah style)
...
<template>
  <div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white p-5 rounded-2xl border border-gray-100 shadow-sm">
      <div>
        <h2 class="text-xl font-bold text-gray-800">Stations Management</h2>
        <p class="text-xs text-gray-500">Add, edit, view, and manage your cashier POS register stations.</p>
      </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
      ...
      <div class="lg:col-span-1 bg-white border border-gray-100 p-6 rounded-2xl shadow-sm h-fit">
        ...
        <form @submit.prevent="handleSubmit" class="space-y-4">
          <!-- Station Name Field -->
          <div class="space-y-1">
            <label class="block text-xs font-bold text-gray-500 uppercase tracking-wider">Station Name *</label>
            <input 
              v-model="form.name"
              type="text" 
          ...
          <!-- IP Address Field -->
          <div class="space-y-1">
            <label class="block text-xs font-bold text-gray-500 uppercase tracking-wider">IP Address</label>
            <input 
              v-model="form.ip_address"
              type="text" 
              placeholder="e.g. 192.168.1.100" 
              class="w-full px-4 py-2 border rounded-xl text-sm focus:outline-none focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500 transition-colors"
              :class="errors.ip_address ? 'border-red-400 focus:border-red-400 focus:ring-red-400' : 'border-gray-200'"
            />
            <p v-if="errors.ip_address" class="text-xs text-red-500 font-medium mt-1">{{ errors.ip_address }}</p>
          </div>

          <!-- Location Field -->
          <div class="space-y-1">
            <label class="block text-xs font-bold text-gray-500 uppercase tracking-wider">Location</label>
            <input 
              v-model="form.location"
              type="text" 
              placeholder="e.g. Front Counter A" 
              class="w-full px-4 py-2 border rounded-xl text-sm focus:outline-none focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500 transition-colors"
              :class="errors.location ? 'border-red-400 focus:border-red-400 focus:ring-red-400' : 'border-gray-200'"
            />
            <p v-if="errors.location" class="text-xs text-red-500 font-medium mt-1">{{ errors.location }}</p>
          </div>

          <!-- Drawer Safety Limit Field -->
          <div class="space-y-1">
            <label class="block text-xs font-bold text-gray-500 uppercase tracking-wider">Drawer Safety Limit (IDR)</label>
            <input 
              v-model="form.drawer_safety_limit"
              type="number" 
              step="any"
          ...
          <div class="flex items-center justify-between p-3 bg-gray-50 rounded-xl">
            <div class="flex flex-col">
              ...
              <span class="text-[10px] text-gray-400">Enable or disable this station.</span>
            </div>
      ...
      <div class="lg:col-span-2 bg-white border border-gray-100 p-6 rounded-2xl shadow-sm flex flex-col min-w-0">
        <h3 class="text-md font-bold text-gray-800 mb-5">Stations Directory</h3>
        ...
        <div v-if="loading" class="flex-1 flex flex-col items-center justify-center py-20 space-y-3">
          ...
          <span class="text-sm font-semibold text-gray-500">Loading stations catalog...</span>
        </div>

        <!-- Empty State -->
        <div v-else-if="filteredStations.length === 0" class="flex-1 flex flex-col items-center justify-center py-20 text-center space-y-4">
          ...
          <div>
            <h4 class="text-sm font-bold text-gray-700">No Stations Found</h4>
            <p class="text-xs text-gray-400 max-w-[280px] mx-auto mt-1">
              {{ searchQuery ? 'No stations match your current search filters.' : 'Your stations list is empty. Create a station on the left to start.' }}
            </p>
          </div>
        </div>
          ...
          <table class="w-full text-left border-collapse text-xs font-semibold text-gray-600">
            <thead>
              <tr class="border-b border-gray-100 text-gray-400 uppercase tracking-wider text-[10px]">
                ...
                <th class="pb-3">Station Info</th>
                <th class="pb-3">IP Address</th>
                <th class="pb-3 text-center">Drawer Limit</th>
                <th class="pb-3 w-20 text-center">Status</th>
                <th class="pb-3 w-24 text-center">Actions</th>
              </tr>
            </thead>
            <tbody class="divide-y divide-gray-50">
              <tr v-for="station in paginatedStations" :key="station.id" class="hover:bg-gray-50/50 transition-colors">
                <!-- Icon column -->
                <td class="py-3">
                  <div class="w-8 h-8 rounded-xl bg-gradient-to-br from-teal-400 to-indigo-500 text-white flex items-center justify-center shadow-sm">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" class="w-4 h-4">
                ...
                <!-- Station Info column -->
                <td class="py-3 pr-4">
                  <span class="text-gray-800 font-bold block">{{ station.name }}</span>
                  <span v-if="station.location" class="text-[10px] text-gray-400 block mt-0.5">{{ station.location }}</span>
                </td>

                <!-- IP Address column -->
                <td class="py-3 pr-4 font-semibold text-gray-600">
                  <span class="px-2 py-0.5 bg-gray-50 border border-gray-100 rounded-md">
                    {{ station.ip_address || 'No IP configured' }}
                  </span>
                </td>

                <!-- Drawer Safety Limit column -->
                <td class="py-3 text-center text-gray-700 font-bold">
                  {{ formatLimit(station.drawer_safety_limit) }}
                </td>
                ...
                <td class="py-3 text-center">
                  <span 
                    ...
                    :class="station.is_active === 1 || station.is_active === true
                  ...
                  >
                    <span 
                      ...
                      :class="station.is_active === 1 || station.is_active === true
                        ...
                        : 'bg-gray-400'"
                    ></span>
                    {{ station.is_active === 1 || station.is_active === true ? 'Active' : 'Inactive' }}
                  </span>
                </td>
        ...
        <!-- Pagination Controls -->
        <div v-if="filteredStations.length > 0" class="mt-6 pt-4 border-t border-gray-100 flex flex-col sm:flex-row justify-between items-center gap-4">
          <!-- Left side: Range info and page size selector -->
          <div class="flex items-center gap-3 text-xs text-gray-500">
            <span>Showing {{ startRange }} to {{ endRange }} of {{ filteredStations.length }} entries</span>
